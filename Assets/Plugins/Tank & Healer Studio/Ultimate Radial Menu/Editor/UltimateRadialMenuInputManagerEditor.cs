...
public class UltimateRadialMenuInputManagerEditor : Editor
{
			...
			else
			{
				...
				if( DisplayCollapsibleBoxSection( "Center Screen Input", "URMIM_CenterScreenInput", serializedObject.FindProperty( "centerScreenInput" ) ) )
				{
					...
					if( targ.virtualReality )
					{
						...
						if( targ.leftEyeCamera == null || targ.rightEyeCamera == null )
	...
	{
		if( targ.touchInput && targ.enableMenuSetting != UltimateRadialMenuInputManager.EnableMenuSetting.Manual && EditorPrefs.GetBool( "URMIM_TouchInput" ) )
		{
